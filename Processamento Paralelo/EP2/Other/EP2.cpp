...
/*	Command being timed: 
	"./EP2 a045b7efa463c6ed195c644163f4168952fbd34a"
...
*/
	...
	if(rank == 0) {
        ...
        cout << "CHEGOU NA TRETA: " << rank << " " << limites[0][0] << ":" << limites[0][1] << endl;
		char s4[5];
		s4[4] = '\0';
			...
			for(int j = 0; j < tam; j++)
				for(int k = 0; k < tam; k++)
					for(int l = 0; l < tam; l++){
						...
						if(!CRIPT.compare(SENHA)){
							cout << "O Processador rank " << i << " achou a Resposta" << endl << RESPOSTA << " : " << SENHA << endl;
							MPI_Abort(MPI_COMM_WORLD, 0);
						}
					}
		cout << "SAIU DA TRETA: " << rank << endl;
        ...
        cout << "CHEGOU NA TRETA: " << rank << " " << limite[0] << ":" << limite[1] << endl;
		char s4[5];
		s4[4] = '\0';
		int tam = strlen(caracteres);
			...
			for(int j = 0; j < tam; j++)
				for(int k = 0; k < tam; k++)
					for(int l = 0; l < tam; l++){
						...
						// cout << s4 << " : " << CRIPT << endl;
						if(!CRIPT.compare(SENHA)){
				            MPI_Send(s4, strlen(s4), MPI_CHAR, 0, 123, MPI_COMM_WORLD);
