...
int **distribui(int **limites, char *c, int np){
	// limites = (int **) calloc(np, sizeof(int *));
...
void processNP(int rank, int *limite, char *caracteres, char *SENHA) {
	...
	// if(rank == 0){
		// cout << "CHEGOU NA TRETA 1: " << rank << " " << limite[0] << ":" << limite[1] << endl;
	    char s1[2];
		s1[1] = '\0';
		...
		// cout << "CHEGOU NA TRETA 2: " << rank << " " << limite[0] << ":" << limite[1] << endl;
	    char s2[3];
		s2[2] = '\0';
		for(int i = limite[0]; i < limite[1]; i++)
			for(int j = 0; j < tam; j++){
		...
		// cout << "CHEGOU NA TRETA 3: " << rank << " " << limite[0] << ":" << limite[1] << endl;
	    char s3[4];
		s3[3] = '\0';
		for(int i = limite[0]; i < limite[1]; i++)
			for(int j = 0; j < tam; j++)
	...
	// } else {
		...
		// cout << "CHEGOU NA TRETA 4: " << rank << " " << limite[0] << ":" << limite[1] << endl;
		char s4[5];
		s4[4] = '\0';
		for(int i = limite[0]; i < limite[1]; i++)
			for(int j = 0; j < tam; j++)
		...
		// cout << "CHEGOU NA TRETA 5: " << rank << " " << limite[0] << ":" << limite[1] << endl;
		char s5[6];
		s5[5] = '\0';
		for(int i = limite[0]; i < limite[1]; i++)
			for(int j = 0; j < tam; j++)
	...
	// cout << "SAIU DA TRETA MALIGNA: " << rank << endl;
}

/*
void processNP6(int rank, MPI_Status status, int *limite, char *caracteres, char *SENHA, char *RESPOSTA){
				...
				for(int l = 0; l < tam; l++)
					for(int m = 0; m < tam; m++)
						for(int n = 0; n < tam; n++){
							...
							if(!CRIPT.compare(SENHA)){
								MPI_Send(s6, strlen(s6), MPI_CHAR, 0, 7, MPI_COMM_WORLD);
								cout << "O Processador rank " << rank << " achou a Resposta" << endl << RESPOSTA << " : " << SENHA << endl;
								MPI_Abort(MPI_COMM_WORLD, 0);
							}
						}
}
...
int main(int argc, char *argv[]) {
	...
	if(rank == 0) {
	    ...
	    #pragma omp parallel sections num_threads(3)
		{
			...
			printf("\nO Processador %d, encontrou a RESPOSTA\t#[%s]\n", status.MPI_SOURCE, resposta);
