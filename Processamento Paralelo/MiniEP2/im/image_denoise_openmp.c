...
int ppma_write ( char *file_out_name, int xsize, int ysize, int *r, int *g, int *b );
int ppma_write_data ( FILE *file_out, int xsize, int ysize, int *r, int *g, int *b );
int ppma_write_header ( FILE *file_out, char *file_out_name, int xsize, int ysize, int rgb_max );
...
int main ( int argc, char *argv[] ){
	...
	int rgb_max;
	FILE *input_unit;
	char input_filename[] = "balloons.ppm";
	char output_filename[] = "balloons_median.ascii.ppm";
...
*/
	for (int i = 0; i < rep; ++i)
{
	input_unit = fopen ( output_filename, "r" );
